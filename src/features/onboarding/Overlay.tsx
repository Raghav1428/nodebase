...
export function Overlay({ step, isVisible, onClickOutside }: OverlayProps) {
  ...
  // Compute values safely even if step/rect is null (handled by conditional render below)
  const showSpotlight = step && rect && step?.showSpotlight !== false;

  const safeRect = showSpotlight && rect ? {
    left: Math.max(rect.left, 4),
    top: Math.max(rect.top, 4),
    width: Math.min(rect.width, viewport.width - Math.max(rect.left, 4) - 4),
    height: Math.min(rect.height, viewport.height - Math.max(rect.top, 4) - 4),
  } : null;

  const maskId = step ? `spotlight-mask-${step.id}` : 'spotlight-mask-default';

  return (
    <AnimatePresence>
      {isVisible && step && (
        <motion.div
          key="overlay" // Stable key for interaction
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
